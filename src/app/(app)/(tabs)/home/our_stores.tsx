...
const StoreLocator = () => {
  ...
  const openFallbackDirections = (store: Store) => {
    const url = Platform.select({
      ios: `maps:${store.latitude},${store.longitude}?q=${encodeURIComponent(store.name)}`,
      android: `geo:${store.latitude},${store.longitude}?q=${store.latitude},${store.longitude}(${encodeURIComponent(store.name)})`,
    ...
    <View style={styles.container}>
      ...
      <View style={styles.mapContainer}>
        <MapView
          ...
          style={styles.map}
          initialRegion={{
            latitude: selectedStore?.latitude || DEFAULT_STORE.latitude,
            longitude: selectedStore?.longitude || DEFAULT_STORE.longitude,
            latitudeDelta: 0.05,
            longitudeDelta: 0.05,
          }}
          showsCompass={true}
