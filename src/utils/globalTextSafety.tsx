 ...
 * This is a comprehensive solution that handles all possible data types safely
 */
 ...
 */
export const getSafeText = (value: any, currentLanguage: string = 'en'): string => {
    if (!value) return '';
...
export function withSafeText<P extends object>(Component: React.ComponentType<P>) {
    return (props: P & { children?: any; currentLanguage?: string }) => {
        ...
        if (children !== undefined) {
            return <Component {...(restProps as P)}>{getSafeText(children, currentLanguage)}</Component>;
        }
...
/**
 * Global text safety wrapper for any value
 * This is the most comprehensive solution that handles all possible data types
 */
export const globalSafeText = (value: any, currentLanguage: string = 'en'): string => {
    try {
        return getSafeText(value, currentLanguage);
    } catch (error) {
        logger.warn('Error in globalSafeText:', error);
        return '[Error]';
    }
};